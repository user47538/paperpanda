...
const elements = {
  ...
  connectAiButton: document.getElementById("connect-ai-button"),
...
function renderAiConnectionState() {
  const isConnected = Boolean(state.settings.openAIApiKey?.trim());
  if (elements.connectAiButton) {
    elements.connectAiButton.textContent = isConnected ? "Update ChatGPT" : "Connect ChatGPT";
  }
  if (elements.aiConnectionStatus) {
    elements.aiConnectionStatus.textContent = isConnected
      ? "ChatGPT connected. AI answers and voice are generated by OpenAI."
      : "ChatGPT not connected. AI answers and voice are generated by OpenAI after connection.";
  }
}
...
async function ensureOpenAiKey(promptText = "Enter your OpenAI API key for this prototype. It will be stored only in this browser.") {
  const existingKey = state.settings.openAIApiKey?.trim();
  if (existingKey) {
    return existingKey;
  }
  ...
  state.settings.openAIApiKey = trimmedKey;
  persistSettings();
...
async function requestOpenAi(endpoint, apiKey, payload, expectBlob = false) {
  let response;
  try {
    response = await window.fetch(`https://api.openai.com/v1/${endpoint}`, {
      method: "POST",
      headers: {
    ...
    if (window.location.protocol === "file:") {
      throw new Error(
        "OpenAI requests are being blocked from this local file. Run this app from a hosted frontend with a backend or API proxy instead of file://."
      );
    }
...
async function requestAskAnswer(question, subject, document) {
  const apiKey = await ensureOpenAiKey(
    "Enter your OpenAI API key to enable ChatGPT answers and the natural female voice in this prototype."
  );
  if (!apiKey) {
    throw new Error("ChatGPT is not connected yet.");
  }
  ...
  const documentContext = document
    ? `Selected document: ${document.title}\nDocument type: ${document.type}\nDocument content:\n${clipText(document.content || "Preview text is not available for this document.")}`
    : "No document is selected for Ask right now.";
  const assessmentContext = nextAssessment
    ? `Next assessment: ${nextAssessment.componentTask || nextAssessment.title}\nDue: ${formatAssessmentDueLabel(nextAssessment.dueDate)}`
    : "No assessment is currently scheduled for this subject.";

  const responsePayload = await requestOpenAi("responses", apiKey, {
  ...
  const answer = extractOpenAiText(responsePayload);
  if (!answer) {
...
function speakDocument(document) {
  ...
  const speakWithOpenAi = async () => {
    const apiKey = await ensureOpenAiKey(
      "Enter your OpenAI API key to enable the natural female AI voice in this prototype."
    );
    if (!apiKey) {
    ...
    const speechBlob = await requestOpenAi(
      "audio/speech",
      apiKey,
      {
        model: OPENAI_TTS_MODEL,
        voice: OPENAI_TTS_VOICE,
        input: clipText(textToRead, 3500),
        instructions:
...
function handleConnectAi() {
  const currentKey = state.settings.openAIApiKey?.trim() || "";
  const enteredKey = window.prompt(
    "Enter your OpenAI API key for this prototype. It will be stored only in this browser so Ask AI and Listen can use OpenAI.",
    currentKey
  );
  if (enteredKey === null) {
    return;
  ...
  if (!trimmedKey) {
    ...
    persistSettings();
    renderAiConnectionState();
    elements.askResponse.textContent = "ChatGPT connection removed.";
    return;
  }
  ...
  persistSettings();
  renderAiConnectionState();
  elements.askResponse.textContent = "ChatGPT connected. Ask a question about the selected document.";
}
...
elements.connectAiButton?.addEventListener("click", handleConnectAi);
